...
argument-hint: <project-name> [description] [output-dir] [--skills-target .claude/skills|.codex/skills|both|none]
...
```
<project-name> [description] [output-dir] [--skills-target .claude/skills|.codex/skills|both|none]
```
...
- `--skills-target` (任意): `lab-seminar-skills` を導入する場所。
  ...
  - `both`: `.claude/skills` と `.codex/skills` の両方
  - `none`: スキル導入をスキップ
  - 省略時: Claude Code で実行中なら `.claude/skills`、Codex で実行中なら `.codex/skills`、判定できない場合は `.claude/skills`
