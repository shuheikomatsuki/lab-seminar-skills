...
- `slug` (任意): URLセーフな識別子（例: `dl-ch6-2`, `bert`, `attention-is-all-you-need`）
  - **省略判定**: 第1引数が `.pdf` で終わるか `/` を含む場合はパスとみなし、slug 省略と判断する
  - **省略時の導出**: PDFファイル名から生成する（拡張子除去・小文字化・非英数字を `-` に置換）
  - 例: `pdfs/bishop-dl-ch6.pdf` → `bishop-dl-ch6`
...
- **失敗した場合**: エラー出力を表示し、原因箇所（nav エントリのインデント、リンク切れ等）を特定して修正してから再実行する
